...
if ( ! function_exists( 'fasindus_dynamic_styles' ) ) {
  function fasindus_dynamic_styles() {

    // Typography
    $fasindus_get_typography  = fasindus_get_typography();
    ...
    // Logo
    $fasindus_logo_top     = cs_get_option( 'fasindus_logo_top' );
    $fasindus_logo_bottom  = cs_get_option( 'fasindus_logo_bottom' );
...
$layout_boxed = ( $bg_pattern === 'custom-pattern' ) ? 'background-image: url('. esc_url($custom_bg_pattern) .');' : 'background-image: url('. esc_url(FASINDUS_IMAGES) . '/patterns/'. $bg_pattern .'.png);';
?>
  .layout-boxed {
    <?php echo   wp_kses_post( $layout_boxed ); ?>
  }
...
if ( $topbar_icon_color ) { ?>
 ...
 .site-header .topbar .social .social-links ul li i:before {
...
if ($fasindus_meta) {
  $transparent_header = $fasindus_meta['transparency_header'];
  $transparent_menu_color = $fasindus_meta['transparent_menu_color'];
  $transparent_menu_hover = $fasindus_meta['transparent_menu_hover_color'];
} else {
  $transparent_header = '';
...
/* Menu CTA - Background */
$header_cta_bgcolor  = cs_get_customize_option( 'header_cta_bgcolor' );
$header_cta_color  = cs_get_customize_option( 'header_cta_color' );
if ( $header_cta_bgcolor ) {?>
  .site-header .search-contact .contact-btn .theme-btn {
    background-color: <?php echo  esc_attr( $header_cta_bgcolor ); ?>;
  }
<?php
}
if ( $header_cta_color ) {?>
...
if ( $footer_bg_color ) {?>
  .site-footer ,
  .site-footer:before {background: <?php echo  esc_attr( $footer_bg_color ); ?>;}
<?php
}
...
if ( $copyright_border_color ) {?>
  .site-footer .lower-footer {border-top: <?php echo  esc_attr( $copyright_border_color ); ?> 1px solid ;}
<?php
}
...
if ( $copyright_link_hover_color ) {?>
  .site-footer .lower-footer p a:hover {color: <?php echo  esc_attr( $copyright_link_hover_color ); ?>;}
<?php
}
...
<?php
  ...
  echo   wp_kses_post( $fasindus_get_typography );
  $output = ob_get_clean();
  return $output;
 ...
 * Custom Font Family
 */
if ( ! function_exists( 'fasindus_custom_font_load' ) ) {
  function fasindus_custom_font_load() {
...
/* Custom Styles */
if( ! function_exists( 'fasindus_custom_css' ) ) {
  function fasindus_custom_css() {
    wp_enqueue_style('fasindus-default-style', get_template_directory_uri() . '/style.css');
    $output  = fasindus_custom_font_load();
    $output .= fasindus_dynamic_styles();

    if( function_exists( 'fasindus_compress_css_lines' ) ) {
      $custom_css = fasindus_compress_css_lines( $output );
       wp_add_inline_style( 'fasindus-default-style', $custom_css );
    }
    fasindus_typography_fonts();
  }
  add_action( 'wp_enqueue_scripts', 'fasindus_custom_css' );
}

/* Custom JS */
if( ! function_exists( 'fasindus_custom_js' ) ) {
  function fasindus_custom_js() {
    $output = cs_get_option( 'theme_custom_js' );
    wp_add_inline_script( 'jquery-migrate', $output );
  }
  add_action( 'wp_enqueue_scripts', 'fasindus_custom_js' );
}