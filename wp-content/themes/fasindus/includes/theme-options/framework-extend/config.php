...
if (function_exists('cs_framework_init')) {
  ...
  require_once( FASINDUS_CS_FRAMEWORK . '/theme-options.php' );
  require_once( FASINDUS_CS_FRAMEWORK . '/theme-customizer.php' );
  require_once( FASINDUS_CS_FRAMEWORK . '/theme-metabox.php' );
   ...
   */
  if( ! function_exists( 'fasindus_custom_font_upload' ) ) {
    function fasindus_custom_font_upload( $db_value ) {
    ...
    add_action( 'cs_typography_family', 'fasindus_custom_font_upload' );
  }
 ...
 * Check Custom Font
 */
if ( ! function_exists( 'fasindus_custom_upload_font' ) ) {
  function fasindus_custom_upload_font( $font ) {
 ...
 */
if ( ! function_exists( 'fasindus_registered_sidebars' ) ) {
  function fasindus_registered_sidebars() {
 ...
 */
if ( ! function_exists( 'fasindus_typography_fonts' ) ) {
  function fasindus_typography_fonts() {
    ...
    if ( ! empty( $embed_fonts ) ) {
      ...
      wp_enqueue_style( 'fasindus-typography-fonts', esc_url( add_query_arg( 'family', urlencode( implode( '|', $query_args ) ) . $subsets, '//fonts.googleapis.com/css' ) ), array(), null );
    }
...
/* Typography */
if ( ! function_exists( 'fasindus_get_typography' ) ) {
  function fasindus_get_typography() {
    ...
    if ( ! empty( $typography ) ) {
      foreach ( $typography as $font ) {
        if ( ! empty( $font['selector'] ) ) {
          ...
          $weight  = ( $font['font']['variant'] != 'regular' ) ? fasindus_esc_string( $font['font']['variant'] ) : 400;
          $style   = fasindus_esc_number( $font['font']['variant'] );
          ...
          $output .= ( ! empty( $font['size'] ) ) ? 'font-size: '. fasindus_check_px( $font['size'] ) .';' : '';
...
if( ! empty( $quomodotheme ) && ! empty( $apikey ) ) {
  require get_template_directory() .'/includes/theme-options/plugins/theme-updater/theme-updater.php';
}