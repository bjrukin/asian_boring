...
/* ==========================================================
  Gmap
=========================================================== */
if ( !function_exists('fasindus_gmap_function')) {
  function fasindus_gmap_function( $atts, $content = NULL ) {
    extract( $atts );
 // Design Tab
    // Atts
    $class = $class ? ' '. $class : '';
    ...
    if ($gmap_common_marker) {
      ...
      $common_marker = FASINDUS_PLUGIN_ASTS . '/images/map-marker.png';
    }
    ...
    // Google Map Height
    if ( $gmap_height ) {
      $inline_style .= '.fasindus-gmap-'. $e_uniqid .' {';
      $inline_style .= ( $gmap_height ) ? 'height:'. fasindus_core_check_px($gmap_height) .';' : '';
      $inline_style .= '}';
    }
    // add inline style
    add_inline_style( $inline_style );
    $styled_class  = ' fasindus-gmap-'. $e_uniqid;
    wp_enqueue_script( 'ifasindusheme-googlemap', FASINDUS_PLUGIN_ASTS . '/jquery.googlemap.js', array( 'jquery' ), '1.5.0', true );
    wp_enqueue_script( 'ifasindusheme-map-api', '//maps.googleapis.com/maps/api/js'. $gmap_api .'', array( 'jquery' ), '', true );
    // Turn output buffer on
    ob_start();
    // Map Tab ID
    if ($gmap_id) {
      $gmap_open = '<div '. $gmap_id .' class="gmsppg">';
      $gmap_close = '</div>';
    } else {
      $gmap_open = '';
      $gmap_close = '';
    ...
    echo $gmap_open;
    echo '<div id="map-'. $e_uniqid .'" class="b-google-map '.esc_attr( $class . $styled_class ).'"></div>';
    echo $gmap_close; ?>
    ...
    jQuery(document).ready(function($) {
      ...
      foreach ( $locations as $location ) {
        ...
        $heading = $location->location_heading ? 'title: "'. esc_js($location->location_heading) .'",' : '';
        $text = $location->location_text ? 'text: "'. esc_js($location->location_text) .'",' : ''; ?>
...
add_shortcode( 'fasindus_gmap', 'fasindus_gmap_function' );
